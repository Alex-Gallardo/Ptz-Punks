...
contract PlatziPunksDNA {
    ...
    // Calculo de ADN/DNA de manera "Random"
    function deterministicPseudoRandomDNA(uint256 _tokenId, address _minter)
        public
        pure
        returns (uint256)
    {
        uint256 combinedParams = _tokenId + uint160(_minter); // uint160: Enteros dedicados a las direcciones
        bytes memory encodedParams = abi.encodePacked(combinedParams); // Convertir nuestro entero en bytes
        bytes32 hashedParams = keccak256(encodedParams); // Funcion interna que nos genera un hash
    ...
    // Obtener la seccion del ADN (La cadena de herencia no debe estar disponible)
    ...
    // Trae el tipo de accesorio
    function getAccessoriesType(uint256 _dna)
        public
        view
        returns (string memory)
    {
        uint8 dnaSection = _getDNASection(_dna, 0); // Si el numero es muy alto, hacemos un recorte
        // La operacion de % da como resultado el resto de una division, no debe confundirse con el calculo de porcentaje.
    ...
    // Configurar el color de la piel
...
// File contracts/PPDNA.sol

pragma solidity >=0.7.0 <0.9.0;

contract PlatziPunks2 is ERC721, ERC721Enumerable, PlatziPunksDNA {
    ...
    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No se puede crear mas platzi punks");
...
contract PunkDNA {
    ...
    // ERROR COMUN:
    // Si hay un error en la sobrecarga de variables, se puede usar el siguiente código:
    // Practicamente es declarar las variables por bloques, ya que el compilador al final genera el codigo en bloques
    // {
    //     params = ...
    // }
